...
llist_t * bg_all_words(boggle_t *bg){
  ...
  llist_t *values = ll_init();
  for (int i = 0; i < hm->num_buckets; i++)
  {
    llist_t * curBucket = hm->buckets[i];
    ll_node_t *curNode = curBucket->head;
    while (curNode != NULL)
    {
      ll_push(values, curNode->val);
...
void _bg_all_words(boggle_t *bg, hashmap_t * hm, int r, int c,
  ...
  if (strlen(cur_word) > 7 || c<0 || r<0 ||  c>=BOGGLE_DIMENSION || r>=BOGGLE_DIMENSION){
    return;
  }
  ...
  if (strlen(cur_word) >= 3 && hm_check(bg->dict,cur_word) && !hm_check(hm, cur_word))
  {
    hm_add(hm,cur_word);
  }
